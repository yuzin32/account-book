...
function nyear_search(formName) {
    const form = document.forms[formName]; // 폼 이름으로 선택
    form.submit(); // submit 함수 호출
}
                    ...
                    <div class="chklist-wrap">
                        ...
                        <div class="chklist-month">
                            <div class="search-area">
                                <div class="search-lab">
                                    ...
                                    <select class="round" name ="search_nyaer" action="/account_book/html/new_tap03/checklist.php"  method="POST">
                                ...
                                <div class="search-lab">
                                    <a href="javascript://" onclick="nyear_search('search_form')" class="search-btn" title="조회">조회</a>
                                </div>
                            </div>
                            <ul class="month-list">
                                <?for($i=1; $i<=12; $i++){
                                    ...
                                    <?if (isset($month_check[$i])){
                                        ...
                                        ?>
                                        <li class="<?=$mclass?>">
                                            <a href="/account_book/html/new_tap03/checklist.php?search_month=<?=$i?>&search_nyaer=<?=$main_year?>" >
                                                <p class="month"><?=$i?>월</p>
                                     ...
                                     <?}else{?><!--체크리스트 데이터가 없음 -->
                                        <li class="<?= $mclass?>">
                                            <a href="/account_book/html/new_tap03/checklist.php?search_month=<?=$i?>">
                                                <p class="month"><?=$i?>월</p>
                                                <p class="task">0 / 0</p>
                        ...
                        <!-- 리스트 start -->
                         <section class="chklist-list">
                           <form  name="c_s_form" action="/account_book/html/new_tap03/checklist.call.php"  method="POST" >
