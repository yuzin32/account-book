	...
	function __readPropertySets(){
		...
		while ($offset < strlen($this->entry)) {
			...
			if ((strtolower($name) == "workbook") || ( strtolower($name) == "book")) {
				$this->wrkbook = ecount($this->props) - 1;
			}
			if ($name == "Root Entry") {
				$this->rootentry = ecount($this->props) - 1;
			}
...
class Spreadsheet_Excel_Reader {
	...
	function dump($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		...
		if ($col_letters) {
			...
			for($i=1;$i<=$this->colecount($sheet);$i++) {
		...
		$out .= "<tbody>\n";
		for($row=1;$row<=$this->rowecount($sheet);$row++) {
			...
			for($col=1;$col<=$this->colecount($sheet);$col++) {
	...
	function _format_value($format,$num,$f) {
		...
		$pattern = $parts[0];
		// Negative pattern
		if (ecount($parts)>2 && $num==0) {
			$pattern = $parts[2];
		}
		// Zero pattern
		if (ecount($parts)>1 && $num<0) {
			$pattern = $parts[1];
			$num = abs($num);
		}
